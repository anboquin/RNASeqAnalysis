#Griffith Lab tutorial with completed exercises at the end
...
dds_cc <- DESeq(dds_cc) #if running parallel processing set parallel = TRUE
...
#use ggplot to construct our own MA plot 
...
ggplot(otop2Counts, aes(x=tissueType, y=count, colour=individualID, group=individualID)) + 
  ...
  guides(colour=guide_legend(ncol=3)) + ggtitle("OTOP2")
    ...
    #primary tumor and metastasis samples compared to the normal 
...
sigGenes <- rownames(dds_ResDF[dds_ResDF$padj <= .05 & abs(dds_ResDF$log2FoldChange) > 3,])
    #used the differential expression results previously obtained to filter the transformed matrix 
    #to only genes which were significantly differentially expressed and with log2 fold change >3 
...
dds2VSTMatrix <- dcast(dds_2VST, Gene ~ variable) #convert transformed values back to wide format 
...
##exercises 
# 1. Create a dendogram for genes use coord_flip() and scale_y_reverse()
?coord_flip #converts geoms and stats which displya y conditional on x, to x conditional on y 
?scale_y_reverse # = transformation 

#use the previously constructed clusterGene object to make a dendrogram 
genesampleModel <- as.dendrogram(clusterGene)
geneDendogramData <- segment(dendro_data(genesampleModel, type = "rectangle"))
geneDendrogram <- ggplot(geneDendogramData) + 
  geom_segment(aes(x=x, y=y, xend=xend, yend=yend)) +
  coord_flip() +
...
#2. re-arrange gene cells in heatmap on the y-axis to match the gene dendogram 
#refactor genes
dds_2VST$Gene <- factor(dds_2VST$Gene,levels = clusterGene$labels[clusterGene$order])
#heatmap 
heatmap <- ggplot(dds_2VST, aes(x=variable, y =Gene, fill=value)) +
  geom_raster() + 
  scale_fill_viridis(trans="sqrt") + 
...
#dendrogram to grob
#my first try 
    #geneDendogram1 <- geneDendogram +
      #scale_x_discrete(expand=c(0, 0))
    #geneDendogramGrob <- ggplotGrob(geneDendogram1)
#in one step:
geneDendrogramGrob <- ggplotGrob(geneDendrogram + 
                                   scale_x_discrete(expand=c(0, 0)))
...
#first try
    #heatmap <- heatmap + 
      #scale_x_discrete(expand=c(0, 0)) + 
      #scale_y_discrete(expand=c(0, 0))
    #heatmapV2Grob <- ggplotGrob(heatmapV2_1)
#in one step:
...
#4. align the gene dendrogram heights to match the heatmap
#check heatmap and gene dendrogram have the same number of vertical elements
length(heatmapGrob$heights) == length(geneDendrogramGrob$heights)
  ##TRUE yay

#check height b/w grobs is the same using unit.pmax
maxHeight <- unit.pmax(geneDendrogramGrob$heights, heatmapGrob$heights)
geneDendrogramGrob$heights <- as.list(maxHeight)
geneDendrogramGrob$heights 
heatmapGrob$heights <- as.list(maxHeight)
heatmapGrob$heights 
  #heights match

##re-align horizontal elements (same steps as in tutorial)
#check widths of the grobs
sampleDendrogramGrob$widths
...
#make sure every width bw grobs is the same 
maxWidth <- unit.pmax(sampleDendrogramGrob$widths, heatmapGrob$widths, sampleClinicalGrob$widths) #finds max widths for each grob
sampleDendrogramGrob$widths <- as.list(maxWidth)#overrides each grobs width with max width
heatmapGrob$widths <- as.list(maxWidth)
sampleClinicalGrob$widths <- as.list(maxWidth)


#5. create a blank panel using the code provided 
blankPanel<-grid.rect(gp=gpar(col="white")) #grid graphics


#6. use the gridExtra package to position the panels and plot the result
#arrange plots
finalGrobV2 <- arrangeGrob(blankPanel, sampleDendrogramGrob, blankPanel, 
                           sampleClinicalGrob, geneDendrogramGrob, heatmapGrob, 
                           ncol=2, nrow=3, widths=c(1,5), heights=c(2,.8,6))
#final result
grid.draw(finalGrobV2)
